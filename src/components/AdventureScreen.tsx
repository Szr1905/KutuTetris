...
export default function AdventureScreen({ theme, currentLevel, onBack, onPlay }: AdventureScreenProps) {
  return (
    <div style={{ minHeight: "100vh", maxHeight: "100vh", overflowY: "auto", background: theme.bgColor, color: theme.textColor, fontFamily: "'Nunito', sans-serif", padding: 20, boxSizing: "border-box" }}>
      <div style={{ width: "100%", maxWidth: 460, margin: "0 auto", paddingBottom: 20 }}>
        ...
        <button onClick={onPlay} style={{ width: "100%", border: "none", borderRadius: 16, padding: "16px 24px", background: theme.accent, color: "#fff", fontFamily: "'Fredoka', sans-serif", fontSize: 22, fontWeight: 900, cursor: "pointer", boxShadow: `0 8px 22px ${theme.accent}55`, marginBottom: 22 }}>Bölüm {currentLevel}'i Oyna</button>
        ...
        <div style={{ background: "rgba(12,24,61,0.72)", borderRadius: 18, padding: 16, marginBottom: 22, border: "1px solid rgba(255,255,255,0.12)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8 }}>
            {[
              { color: "#28c9ef", count: 0, label: "Mavi" },
              { color: "#58d63c", count: 0, label: "Yeşil" },
              { color: "#f5a623", count: 0, label: "Turuncu" },
              { color: "#ffd447", count: 0, label: "Yıldız" },
              { color: "#d848e8", count: 0, label: "Pembe" },
            ].map((task) => (
